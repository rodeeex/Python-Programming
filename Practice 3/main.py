...
def left_leaf(value: float) -> float:
    """
    Вычислить значение левого потомка элемента дерева.

    :param value: текущее значение элемента
...
def right_leaf(value: float) -> float:
    """
    Вычислить значение правого потомка элемента дерева.
...
def helper():
    """
    Функция, преобразующая пользовательский ввод в параметры для построения и вывода дерева.

    :return: кортеж из словаря с именованными параметрами и флаг использования рекурсии при выводе дерева как словаря
    :raise ValueError: некорректные: значение корня дерева / высота дерева /
     флаг использования стандартных функций листьев / флаг рекурсии при построении дерева /
     флаг рекурсии при выводе дерева в виде словаря
    """
...
def main():
    """
    Основная функция программы. Вызывает helper(), строит дерево и выводит его в виде словаря и в виде списка.
    """
    params = helper()
